...
export class PreloadService {
  ...
  async preloadCustomerData(apiKey: string): Promise<void> {
    try {
      ...
      if (this.preloadPromises.has(customerId)) {
        console.log(`[Preload] Already preloading for customer ${customerId}, waiting...`);
      ...
      try {
        ...
        console.log(`[Preload] Customer ${customerId} preload completed in ${duration}ms`);
      } finally {
  ...
  private async doPreload(apiKey: string, customerId: number): Promise<void> {
    ...
    console.log(`[Preload] Loading campaigns for top ${agentsToPreload.length} brand agents`);
    ...
    for (let i = 0; i < agentsToPreload.length; i += batchSize) {
      ...
      const campaignPromises = batch.map(async (agent) => {
        try {
          ...
          console.log(`[Preload] Agent ${agent.id}: ${allCampaigns.length} total, ${activeCampaigns.length} active campaigns`);
          ...
          if (this.config.loadRecentCampaigns && allCampaigns.length > 0) {
            ...
            console.log(`[Preload] Loaded details for ${recentCampaigns.length} recent campaigns for agent ${agent.id}`);
          }
          
          return { agentId: agent.id, campaignCount: allCampaigns.length };
          
        } catch (err) {
          console.warn(`[Preload] Failed to load campaigns for agent ${agent.id}:`, err);
      ...
      console.log(`[Preload] Batch ${Math.floor(i / batchSize) + 1}: ${successful}/${batch.length} agents processed`);
    }
    ...
    const topAgentsForDetails = brandAgents.slice(0, Math.min(10, brandAgents.length));
    ...
    console.log(`[Preload] Loaded individual details for ${topAgentsForDetails.length} brand agents`);
  }
