...
export class CachedBigQuery extends BigQuery implements CacheService {
  ...
  invalidatePattern(pattern: string): void {
    ...
    console.log(
      `[Cache] Invalidated ${invalidated} entries matching pattern: ${pattern}, freed ${(freedMemory / 1024).toFixed(1)}KB`,
    );
  }
  ...
  // Override the query method to add caching - support BigQuery overloads
  async query(...args: any[]): Promise<any> {
    // Handle multiple signatures of BigQuery.query method
    let queryOptions: any;
    const firstArg = args[0];
    if (typeof firstArg === "string") {
      queryOptions = { query: firstArg, ...args[1] };
    } else {
    ...
    if (this.inFlightRequests.has(cacheKey)) {
      console.log(
        `[Cache] Waiting for in-flight request: ${cacheKey.substring(0, 50)}...`,
      );
  ...
  private cleanupExpired(): void {
    ...
    if (cleaned > 0) {
      console.log(
        `[Cache] Cleaned up ${cleaned} expired entries, freed ${(freedMemory / 1024).toFixed(1)}KB. Cache size: ${this.cache.size}`,
      );
    }
  }
  ...
  private determineTTL(query: string): number {
    ...
    if (
      this.isQueryForTable(queryLower, "brand_agents") ||
      this.isQueryForTable(queryLower, "public_agent")
    ) {
  ...
  private enforceMemoryLimit(newEntrySize: number): void {
    ...
    while (
      this.currentMemoryBytes + newEntrySize > maxBytes &&
      this.cache.size > 0
    ) {
      this.evictLRUEntry();
    }
  ...
  private evictLRUEntry(): void {
    ...
    if (lruKey) {
      ...
      console.log(
        `[Cache] Evicted LRU entry for memory limit: ${lruKey.substring(0, 50)}...`,
      );
    }
  }
  ...
  private async fetchAndCache(args: any[], cacheKey: string): Promise<any> {
    ...
    try {
      ...
      const queryString =
        typeof options === "string" ? options : (options as any)?.query || "";
      ...
      console.log(
        `[Cache] Cached result (${queryTime}ms, TTL: ${ttl}ms, Size: ${(entrySize / 1024).toFixed(1)}KB): ${cacheKey.substring(0, 50)}...`,
      );
      return result;
    } catch (error) {
      console.error(
        `[Cache] Query failed (${Date.now() - startTime}ms): ${cacheKey.substring(0, 50)}...`,
        error,
      );
      throw error;
    }
  ...
  private generateCacheKey(options: any): string {
    ...
    const keyPrefix = customerContext
  ...
  private isCreativeAssignmentQuery(query: string): boolean {
    return (
      query.includes("campaign_creatives") ||
  ...
  private isQueryForTable(query: string, tableName: string): boolean {
    ...
    const updatePattern = new RegExp(
      `\\bupdate\\s+[\`"']?${tableName}[\`"']?\\b`,
      "i",
    );
    const insertPattern = new RegExp(
      `\\binsert\\s+into\\s+[\`"']?${tableName}[\`"']?\\b`,
      "i",
    );

    return (
      fromPattern.test(query) ||
      joinPattern.test(query) ||
      updatePattern.test(query) ||
      insertPattern.test(query)
    );
  }
