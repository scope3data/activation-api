...
export class AdTechPreloadService {
  ...
  async preloadCustomerData(apiKey: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(`[AdTechPreload] Customer ${customerId} preload completed in ${duration}ms`);
  ...
  private async doAdTechPreload(apiKey: string, customerId: number): Promise<void> {
  ...
  private async loadAndPrioritizeBrandAgents(customerId: number) {
    console.log(`[AdTechPreload] Loading brand agents for customer ${customerId}`);
    ...
    console.log(`[AdTechPreload] Prioritizing ${brandAgents.length} brand agents by activity`);
    return brandAgents.slice(0, this.config.maxBrandAgents);
  }
   ...
   */
  private async preloadActiveCampaigns(apiKey: string, brandAgents: any[]): Promise<void> {
    ...
    for (let i = 0; i < brandAgents.length; i += batchSize) {
      ...
      await Promise.allSettled(batch.map(async (agent) => {
        try {
          ...
          console.log(`[AdTechPreload] Agent ${agent.id}: ${activeCampaigns.length} active, ${allCampaigns.length} total campaigns`);
          
          // Load individual campaign details for active campaigns
          if (activeCampaigns.length > 0) {
            const campaignsToDetail = activeCampaigns.slice(0, this.config.maxCampaignsPerAgent);
          ...
          console.warn(`[AdTechPreload] Failed to load campaigns for agent ${agent.id}:`, err);
        }
      }));
    }
  }
   ...
   */
  private async warmBudgetTrackingData(apiKey: string, brandAgents: any[]): Promise<void> {
      ...
      try {
        ...
        const budgetPromises = activeCampaigns.map(async (campaign) => {
          if (this.config.loadBudgetData) {
            ...
            console.log(`[AdTechPreload] Would warm budget data for campaign ${campaign.id}`);
          }
          
          if (this.config.loadSpendData && this.config.warmRecentSpendData) {
            // Trigger recent spend queries (these will be cached)
            console.log(`[AdTechPreload] Would warm spend data for campaign ${campaign.id}`);
          }
        });
        ...
        console.warn(`[AdTechPreload] Failed to warm budget data for agent ${agent.id}:`, err);
      }
    }
  }
   ...
   */
  private async warmCreativeAssignments(apiKey: string, brandAgents: any[]): Promise<void> {
    console.log('[AdTechPreload] Warming creative assignments');
    
    // This would need creative assignment querying methods
    // For now, log the intent
    for (const agent of brandAgents) {
      console.log(`[AdTechPreload] Would warm creative assignments for brand agent ${agent.id}`);
    }
  }
   ...
   */
  private async warmBrandStoryAssignments(apiKey: string, brandAgents: any[]): Promise<void> {
    console.log('[AdTechPreload] Warming brand story assignments');
    
    // This would need brand story assignment querying methods
    for (const agent of brandAgents) {
      console.log(`[AdTechPreload] Would warm brand story assignments for brand agent ${agent.id}`);
    }
  }
  ...
  private schedulePeriodicRefresh(apiKey: string, customerId: number): void {
    ...
    const timer = setInterval(async () => {
      console.log(`[AdTechPreload] Periodic refresh for customer ${customerId}`);
      
      try {
        // Only refresh critical data, not full preload
        await this.refreshCriticalData(apiKey, customerId);
      } catch (err) {
        console.error(`[AdTechPreload] Periodic refresh failed for customer ${customerId}:`, err);
      }
    }, refreshMs);
    
    this.refreshIntervals.set(customerId, timer);
    console.log(`[AdTechPreload] Scheduled periodic refresh every ${this.config.preloadFrequency} minutes for customer ${customerId}`);
  }
  
  /**
   * Refresh only critical, fast-changing data
   */
  private async refreshCriticalData(apiKey: string, customerId: number): Promise<void> {
    const brandAgents = await this.campaignService.listBrandAgents(customerId);
    ...
    console.log(`[AdTechPreload] Refreshed critical data for ${topAgents.length} top brand agents`);
  }
  ...
  cleanup(): void {
    for (const [customerId, timer] of this.refreshIntervals.entries()) {
      clearInterval(timer);
      console.log(`[AdTechPreload] Cleaned up refresh timer for customer ${customerId}`);
    }
