...
describe("get_prebid_segments Tool", () => {
  let mockService: { getPrebidSegments: vi.MockedFunction<(orgId: string) => Promise<PrebidSegment[]>> };
  ...
  describe("Input Validation", () => {
    it("should validate required orgId parameter", async () => {
      ...
      await expect(
        tool.execute({ orgId: "" }, mockContext),
      ).rejects.toThrow(/organization id is required/i);
    });
    ...
    it("should accept valid orgId", async () => {
      ...
      expect(result).toContain("No active campaigns targeting publisher org valid_org_123");
      expect(mockService.getPrebidSegments).toHaveBeenCalledWith("valid_org_123");
    });
  });
  ...
  describe("Service Integration", () => {
    it("should handle empty segment results", async () => {
      ...
      expect(result).toContain("No active campaigns targeting publisher org empty_org");
    ...
    it("should handle successful segment retrieval", async () => {
      const mockSegments: PrebidSegment[] = [
        ...
        { axe_include_segment: "axe_low_value_789", max_cpm: 4.50 },
      ];
      ...
      expect(result).toContain("Found **3** active campaign targeting segments");
      ...
      expect(result).toContain("Pass these segments to your ad server as **inclusion targeting**");
      expect(result).toContain("Higher CPM segments should get priority in auction logic");
    });
    ...
    it("should handle service errors gracefully", async () => {
      ...
      ).rejects.toThrow("Failed to get prebid segments: BigQuery connection failed");
    });
  ...
  describe("Response Formatting", () => {
    it("should format single segment correctly", async () => {
      ...
      expect(result).toContain("Found **1** active campaign targeting segments");
    ...
    it("should prioritize segments correctly", async () => {
      ...
      const segment1Line = lines.find(line => line.includes("Priority: Highest"));
      const segment2Line = lines.find(line => line.includes("axe_segment_2") && line.includes("Priority: High"));
      const segment3Line = lines.find(line => line.includes("axe_segment_3") && line.includes("Priority: High"));
      const segment4Line = lines.find(line => line.includes("axe_segment_4") && line.includes("Priority: Standard"));
