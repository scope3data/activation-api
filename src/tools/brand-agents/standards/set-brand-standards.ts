...
export const setBrandStandardsTool = (client: Scope3ApiClient) => ({
    ...
    try {
      ...
      if (Object.keys(filteredInput).length === 0) {
        return createMCPResponse({
          message:
            "No brand standards specified. Please provide at least one standard to set (domainBlocklist, domainAllowlist, keywordFilters, or contentCategories).",
          success: false,
        });
      }
  ...
  parameters: z.object({
    brandAgentId: z
      .string()
      .describe("ID of the brand agent to set standards for"),
    contentCategories: z
      .array(z.string())
      .optional()
      .describe(
        "Content categories to avoid (e.g., ['adult', 'violence', 'drugs'])",
      ),
    domainAllowlist: z
      .array(z.string())
      .optional()
    ...
    domainBlocklist: z
      ...
      .optional()
      .describe(
        "List of domains to block (e.g., ['competitor.com', 'inappropriate-site.com'])",
      ),
    keywordFilters: z
      .array(z.string())
      .optional()
      .describe(
        "Keywords to filter out from content (e.g., ['gambling', 'politics'])",
      ),
  }),
});
