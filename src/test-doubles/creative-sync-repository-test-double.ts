 ...
 */
export class CreativeSyncRepositoryTestDouble implements CreativeSyncRepository {
  ...
  async updateSyncStatus(
    ...
    if (!creativeId || !salesAgentId || !brandAgentId) {
      throw new Error("creativeId, salesAgentId, and brandAgentId are required");
    }

    // Validate enum values
    if (updates.syncStatus && !["pending", "syncing", "synced", "failed", "not_applicable"].includes(updates.syncStatus)) {
      throw new Error(`Invalid syncStatus: ${updates.syncStatus}`);
    }

    if (updates.approvalStatus && !["pending", "approved", "rejected", "changes_requested"].includes(updates.approvalStatus)) {
    ...
    if (existing) {
      ...
      existing.approvalStatus = updates.approvalStatus ?? existing.approvalStatus;
      existing.syncError = updates.syncError === null ? undefined : (updates.syncError ?? existing.syncError);
      existing.rejectionReason = updates.rejectionReason ?? existing.rejectionReason;
      existing.requestedChanges = updates.requestedChanges ?? existing.requestedChanges;
      existing.lastSyncAttempt = updates.lastSyncAttempt ?? existing.lastSyncAttempt;
      ...
      const newRecord: SyncStatusRecord = {
        ...
        tacticContext: updates.tacticContext,
        campaignContext: updates.campaignContext,
        createdAt: now,
        updatedAt: now,
      };
  ...
  async getCreativeSyncStatus(creativeId: string): Promise<CreativeSyncStatus[]> {
    ...
    return results.sort((a, b) => a.salesAgentName.localeCompare(b.salesAgentName));
  }

  async getBatchCreativeSyncStatus(creativeIds: string[]): Promise<Record<string, CreativeSyncStatus[]>> {
  ...
  async getSalesAgentCapabilities(salesAgentId: string): Promise<SalesAgentCapabilities | null> {
    const agent = this.salesAgents.get(salesAgentId);
    return agent ? agent.capabilities : null;
  }

  async getBatchSalesAgentCapabilities(salesAgentIds: string[]): Promise<Record<string, SalesAgentCapabilities>> {
  ...
  async isFormatCompatible(creativeFormat: string, salesAgentId: string): Promise<boolean> {
    const agent = this.salesAgents.get(salesAgentId);
    if (!agent) {
      return false;
    }
  ...
  private isFormatCompatibleSync(creativeFormat: string, capabilities: SalesAgentCapabilities): boolean {
  ...
  async getSyncStatistics(
    ...
    const filteredStatuses = Array.from(this.syncStatuses.values()).filter(status => {
      if (status.brandAgentId !== brandAgentId) return false;
      
      if (options?.campaignId && status.campaignContext !== options.campaignId) {
        return false;
      }
      
      if (options?.dateRange) {
        if (status.createdAt < options.dateRange.start || 
    ...
    const syncedCreatives = new Set(filteredStatuses.filter(s => s.syncStatus === "synced").map(s => s.creativeId));
    const approvedCreatives = new Set(filteredStatuses.filter(s => s.approvalStatus === "approved").map(s => s.creativeId));
    const rejectedCreatives = new Set(filteredStatuses.filter(s => s.approvalStatus === "rejected").map(s => s.creativeId));
    const failedSyncs = filteredStatuses.filter(s => s.syncStatus === "failed").length;
