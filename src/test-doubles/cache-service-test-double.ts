import type { CacheService, CacheEntry, CacheStats, QueryOptions } from "../contracts/cache-service.js";
...
export class CacheServiceTestDouble implements CacheService {
  ...
  invalidatePattern(pattern: string): void {
    ...
    for (const [key, entry] of this.cache) {
      if (entry.queryPattern && entry.queryPattern.toLowerCase().includes(pattern.toLowerCase())) {
  ...
  private async simulateQueryExecution(options: QueryOptions): Promise<void> {
    if (this.config?.simulateErrors && Math.random() < 0.1) {
      throw new Error(`Simulated database error for query: ${options.query.substring(0, 50)}`);
    }
    
    // Simulate network/database delay
    await new Promise(resolve => setTimeout(resolve, this.queryExecutionDelay));
  }
  ...
  private determineTTL(query: string): number {
    ...
    if (queryLower.includes('brand_agent') || queryLower.includes('public_agent')) {
