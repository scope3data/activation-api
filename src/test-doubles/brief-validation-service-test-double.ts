 ...
 */
export class BriefValidationServiceTestDouble implements IBriefValidationService {
  ...
  private evaluateBrief(brief: string): {
    ...
    if (!brief || brief.length === 0) {
      return {
        score: 0,
        feedback: "No brief content provided. Publishers need critical information to match inventory.",
        suggestions: [
    ...
    const hasObjectives = /\b(goal|objective|aim|target|increase|improve|drive|boost|achieve|generate|awareness|conversion|engagement)\b/i.test(brief);
    const hasMetrics = /\b(metric|kpi|measure|track|roi|ctr|impression|click|conversion|lift|reach|frequency|awareness|download|visit)\b/i.test(brief);
    const hasAudience = /\b(audience|demographic|customer|user|people|age|gender|millennial|adult|consumer|target)\b/i.test(brief);
    const hasGeo = /\b(geographic|location|city|state|country|region|market|usa|united states|uk|nyc|la|chicago|tier|metro|national|local|sf|austin)\b/i.test(brief);
    ...
    const hasCreativeSpecs = /\b(creative|video|banner|display|ad|content|format|size|messaging|15s|30s|728x90|300x250|native|html5|image|testimonial)\b/i.test(brief);
