...
describe('Asset Upload Error Scenarios', () => {
  ...
  const validAsset = {
    ...
    base64Data: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAHNrEkSsAAAAABJRU5ErkJggg==',
  ...
  describe('Input Validation Errors', () => {
    ...
    it('should handle too many assets in single request', async () => {
      ...
      const tooManyAssets = Array(11).fill(null).map((_, i) => ({
        ...validAsset,
        filename: `test-${i}.png`
      }));
  ...
  describe('Rate Limiting Scenarios', () => {
    it('should handle rate limit exceeded', async () => {
      ...
      const rapidRequests = Array(60).fill(null).map(() => 
  ...
  describe('GCS Connection Errors', () => {
    it('should handle GCS connection timeout', async () => {
      ...
      mockStorageService.prototype.uploadAsset = vi.fn().mockImplementation(() => {
    ...
    it('should handle GCS service unavailable', async () => {
      ...
      const mockStorageService = vi.mocked(AssetStorageService);
      mockStorageService.prototype.uploadAsset = vi.fn().mockRejectedValue(
  ...
  describe('Circuit Breaker Scenarios', () => {
    it('should open circuit breaker after repeated failures', async () => {
      ...
      const mockStorageService = vi.mocked(AssetStorageService);
      mockStorageService.prototype.uploadAsset = vi.fn().mockRejectedValue(
        new Error('Persistent GCS failure')
      );
      ...
      const failurePromises = Array(6).fill(null).map(() => 
  ...
  describe('Concurrent Upload Scenarios', () => {
    it('should handle concurrent uploads without corruption', async () => {
      ...
      mockStorageService.prototype.uploadAsset = vi.fn().mockImplementation(async () => {
        uploadCount++;
