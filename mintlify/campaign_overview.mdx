...
3. **Audience Profiles (Optional)** - Synthetic audience targeting profiles
...
<Tabs>
  <Tab title="Conversational Creation">
    ...
    Claude: 🚀 Creating Nike campaign...
            ...
            📺 Strategy: INTELLIGENT_PMPS optimization
...
Once launched, campaigns automatically begin the **INTELLIGENT_PMPS** optimization process:

1. **Signal Discovery** - Algorithm identifies high-performing audience signals
2. **Story Optimization** - Narrative approaches are tested and refined
...
<CardGroup cols={3}>
  ...
  <Card title="Data Export" href="/api-reference/reporting/export-campaign-data" icon="database">
    **Enterprise Analytics**
    ...
    Export structured data for BI systems with flexible grouping by tactic, signal, story, and publisher.
  </Card>
  
  <Card title="ML Analytics" href="/api-reference/reporting/analyze-tactics" icon="microscope">
    **Advanced Optimization**
    
    Deep statistical analysis of tactic performance with actionable optimization recommendations.
  </Card>
</CardGroup>

## Key Campaign Features

### 🎯 INTELLIGENT_PMPS Strategy

All campaigns use Scope3's **INTELLIGENT_PMPS** (Intelligent Programmatic Private Marketplace) strategy:

- **Private Marketplace Focus**: Premium, curated inventory
...
Track how different audience signals (age, interests, behavior) perform across tactics:
...
#### Story Optimization
Understand which narrative approaches drive the best performance:

```javascript
const storyAnalysis = await analyzeTactics({
  campaignId: "camp_123",
  analysisType: "stories",
  compareStories: true
});

// Returns story performance breakdown:
// - "performance" narrative: 15% higher CTR
// - "lifestyle" narrative: 32% better completion rates
// - "value" narrative: 8% lower CPM
```
...
- Leverage synthetic audiences for precise targeting
...
- Use ML insights from `analyze_tactics` for optimization decisions
...
const analysisA = await analyzeTactics({
  campaignId: campaignA.id,
  analysisType: "efficiency"
});

const analysisB = await analyzeTactics({
