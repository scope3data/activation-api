...
Skip the complex setup - add our MCP server to any AI assistant and let it implement everything for you:
...
## 🤖 One-Click Setup for Popular AI Agents
...
<Tabs>
  ...
  <Tab title="Claude Code">
    ...
    **2. In Claude Code:** *"Connect to https://docs.agentic.scope3.com/mcp and help me optimize our Q4 campaign performance"*
...
The Scope3 Campaign API includes a built-in MCP (Model Context Protocol) server that enables natural language campaign management through any AI assistant.
...
# Configure to connect to https://api.agentic.scope3.com/mcp
```
...
→ Uses create_brand_agent tool
→ Returns: "✅ Created brand agent 'Nike Sustainability' (ba_abc123)"
```
...
→ Uses create_campaign with proper parameters
→ Returns: "🚀 Campaign 'Summer Sustainability 2024' launched with $50K budget"
```
...
→ Uses discover_publisher_products with filters
→ Returns: List of matching inventory options with pricing
```
...
3. **Header-based** (for HTTP requests):
   ```bash
   curl -H "x-scope3-api-key: your_key" https://api.agentic.scope3.com/mcp
   ```
...
4. **Create brand stories** - Build targeting profiles for campaign optimization
