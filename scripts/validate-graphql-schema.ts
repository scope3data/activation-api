...
class GraphQLSchemaValidator {
  ...
  private loadSchema(): void {
    try {
      ...
      console.error(`❌ Failed to load GraphQL schema: ${error instanceof Error ? error.message : String(error)}`);
      process.exit(2);
    }
  }
  ...
  private async validateFile(filePath: string): Promise<ValidationResult[]> {
    ...
    try {
      ...
      for (const [operationName, operation] of operations.entries()) {
        const result = this.validateOperation(filePath, operationName, operation);
        results.push(result);
      }
    } catch (error) {
      console.warn(`⚠️ Failed to read file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private extractGraphQLOperations(content: string): Map<string, string> {
    ...
    const operationRegex = /export\s+const\s+([A-Z_][A-Z0-9_]*(?:QUERY|MUTATION))\s*=\s*`([^`]+)`/g;
   ...
   */
  private validateOperation(file: string, operationName: string, operation: string): ValidationResult {
    const operationType = operationName.includes("MUTATION") ? "mutation" : "query";
   ...
   */
  private generateSuggestions(operationType: "query" | "mutation", errors: GraphQLError[]): string[] {
    ...
    for (const error of errors) {
      ...
      if (message.includes("field") && message.includes("doesn't exist")) {
        ...
        if (fieldMatch) {
          ...
          const alternatives = this.findAlternativeFields(operationType, fieldName);
          
          if (alternatives.length > 0) {
            suggestions.push(`Consider using one of these fields instead: ${alternatives.join(", ")}`);
          }
        }
      }
      
      if (message.includes("brandagent") && message.includes("doesn't exist")) {
        suggestions.push("Try using 'agents' instead of 'brandAgents' for listing operations");
        suggestions.push("Try using 'agent(id: String!)' instead of 'brandAgent(id: String!)' for single queries");
      }
      
      if (message.includes("tactic")) {
        suggestions.push("Tactic operations are not available in GraphQL - use BigQuery service instead");
        suggestions.push("Remove GraphQL tactic operations and implement BigQuery-only approach");
      }
    }
   ...
   */
  private findAlternativeFields(operationType: "query" | "mutation", requestedField: string): string[] {
    const type = operationType === "query" ? this.schema.getQueryType() : this.schema.getMutationType();
    ...
    for (const field of fields) {
      ...
      if (fieldLower.includes(requestedLower.substring(0, Math.min(5, requestedLower.length))) ||
          requestedLower.includes(fieldLower.substring(0, Math.min(5, fieldLower.length)))) {
  ...
  printResults(results: ValidationResult[]): void {
    ...
    console.log(`📁 Total files scanned: ${new Set(results.map(r => r.file)).size}`);

    if (validResults.length > 0) {
      console.log(`\n✅ Valid Operations:`);
      for (const result of validResults) {
        console.log(`   • ${result.operation} (${result.type}) in ${result.file}`);
      }
    }
    ...
    if (invalidResults.length > 0) {
      ...
      console.log(`   Run with --fix-mode to get automated fixes (when available).`);
    } else {
   ...
   */
  getAvailableOperations(): { queries: string[]; mutations: string[] } {
    const queryType = this.schema.getQueryType();
    const mutationType = this.schema.getMutationType();

    return {
      queries: queryType && "getFields" in queryType ? Object.keys(queryType.getFields()).sort() : [],
      mutations: mutationType && "getFields" in mutationType ? Object.keys(mutationType.getFields()).sort() : []
    };
  }
  ...
  printAvailableOperations(): void {
    const { queries, mutations } = this.getAvailableOperations();
...
export { GraphQLSchemaValidator, type ValidationResult, type SchemaValidationConfig };