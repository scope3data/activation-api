...
async function testRealUpload() {
  ...
  if (!existsSync(imageFile)) {
    ...
    log('yellow', '💡 Make sure wonderstruck_shroom_300x250.jpg is in the current directory');
    return;
  }
  ...
  // Check credentials
  const credentialsPath = process.env.GOOGLE_APPLICATION_CREDENTIALS || './gcs-credentials.json';
  if (!existsSync(credentialsPath)) {
    log('red', `❌ GCS credentials not found: ${credentialsPath}`);
    log('yellow', '💡 Run ./setup-gcs.sh to create service account credentials');
    return;
  }
  ...
  try {
    ...
    console.log(`   • Base64 length: ${base64Data.length.toLocaleString()} characters`);
    ...
    console.log(`   • File Size: ${uploadResult.fileSize.toLocaleString()} bytes`);
    ...
    if (error.message.includes('authentication')) {
      log('yellow', '💡 Authentication issue - check your service account credentials');
    } else if (error.message.includes('bucket')) {
      log('yellow', '💡 Bucket issue - make sure the bucket exists and has proper permissions');
    } else if (error.message.includes('permission')) {
      log('yellow', '💡 Permission issue - ensure service account has Storage Admin role');
    }
