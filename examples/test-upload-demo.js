...
try {
  ...
  console.log(
    `   • File size: ${imageBuffer.length} bytes (${(imageBuffer.length / 1024).toFixed(1)} KB)`,
  );
  ...
  console.log(
    `   • Public URL: https://storage.googleapis.com/scope3-creative-assets/${gcsPath}`,
  );
  ...
  console.log(
    "   1. Copy public URL: https://storage.googleapis.com/scope3-creative-assets/" +
      gcsPath,
  );
  ...
  if (error.code === "ENOENT" && error.path?.includes("wonderstruck")) {
    ...
    console.log(
      "   This demo expects the image file to be in the current directory.",
    );
    console.log(
      "   You can use any image file and rename it, or create a test file.",
    );
  }
}
